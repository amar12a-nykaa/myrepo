...
#    customer_2_products_purchased = {row['customer_id']: row['products'] for row in c2p_df.select(['customer_id', 'order_id', 'product_id', 'order_date']).distinct().withColumn('rank', func.dense_rank().over(Window.partitionBy('customer_id').orderBy(desc('order_date')))).filter(col('rank') <= orders_count).groupBy('customer_id').agg(func.collect_list('product_id').alias('products')).collect()}
    ...
    if limit:
        customer_ids = list(c2p_df.agg(func.collect_set('customer_id')).collect()[0]['collect_set(customer_id)'])
        customer_ids = customer_ids[:limit]


    c2p_df = c2p_df.filter(col('customer_id').isin(customer_ids)).select(['customer_id', 'order_id', 'product_id', 'order_date']).distinct().withColumn('rank', func.dense_rank().over(Window.partitionBy('customer_id').orderBy(desc('order_date')))).filter(col('rank') <= orders_count).select(['customer_id', 'product_id']).distinct().withColumnRenamed('product_id', 'product')
