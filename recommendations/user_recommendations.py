...
def compute_recommendation_rows(customer_ids, customer_2_product_chunks, recommendation_rows, product_2_recommendations):
    for customer_id in customer_ids:
        recommendation_rows.append((customer_id, 'user', 'bought', 'coccurence_direct', json.dumps(RecommendationsUtils.customersAlsoBoughtWithMultipleProductsBuckets(customer_2_product_chunks[customer_id], 'coccurence_direct', product_2_recommendations=product_2_recommendations))))
...
def process_orders_df(start_datetime=None):
    ...
    Parallel(n_jobs=10, verbose=1, pre_dispatch='1.5*n_jobs', backend="threading")(delayed(compute_recommendation_rows)(customer_ids_chunk, customer_2_product_chunks, recommendation_rows, product_2_recommendations) for customer_ids_chunk in customer_ids_chunks)

    print("Total Recommendation rows: %d" % len(recommendation_rows))
    recommendation_rows_chunks = [recommendation_rows[i:i+100] for i in range(0, len(recommendation_rows), 100)]
