...
def create_missing_indices():
    ...
    if 'date_1_platform_1_term_1' not in [x['name'] for x in indices]:
        search_terms_daily.create_index(
            [("date", pymongo.ASCENDING), ("platform", pymongo.ASCENDING), ("term", pymongo.ASCENDING)])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--startdate", help="startdate in YYYYMMDD format or number of days to add from today i.e -4",
                        type=CliUtils.valid_date, default=arrow.now().replace(days=-30).format('YYYY-MM-DD'))
    parser.add_argument("--enddate", help="enddate in YYYYMMDD format or number of days to add from today i.e -4",
                        type=CliUtils.valid_date, default=arrow.now().replace().format('YYYY-MM-DD'))
    argv = vars(parser.parse_args())
    if argv['filepath']:
        read_file(filepath=argv['filepath'], platform=argv['platform'], dryrun=argv['dryrun'], limit=argv['limit'],
                  product_id=argv['id'], debug=argv['debug'])
    else:
        print(argv['startdate'])
        read_file_by_dates(startdate=argv['startdate'], enddate=argv['enddate'], platform=argv['platform'],
                           dryrun=argv['dryrun'], limit=argv['limit'], product_id=argv['id'], debug=argv['debug'])
