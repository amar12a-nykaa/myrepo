...
DAILY_SYNC_FILE_PREFIX= 'website_data_all_metrics_'
...
def prepare_views_ca_dataframe(files):
    ...
    df = df.withColumnRenamed("Date", "date")
        #.withColumnRenamed("Customer ID (evar23)", "customer_id")
        .withColumnRenamed("Products", "product_id")
        .withColumnRenamed("Product Views", "views")
        .withColumnRenamed("Cart Additions", "cart_additions")
...
def generate_trending_products_from_CA(env_details,floating=False, algo="ca", days=15, limit=None, views=False, threshold=2, latest_threshold=10):
...
if __name__ == '__main__':
    ...
    generate_trending_products_from_CA(env_details, views=True, threshold=-1, latest_threshold=10)
