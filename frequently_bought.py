...
def prepare_cursor_for_current_orders_table():
    ...
    if DEBUG_MODE:
        ...
        orders_query = "SELECT sfoi.order_id, sfoi.product_id, sfoi.product_type, ccpi.category_id, eaov.value " \
                       ...
                       "WHERE (sfoi.order_id <> 0 AND sfoi.mrp > 1 AND sfoi.order_id in (" + order_ids_str + ") AND ccpi.category_id IN (" + l1_categories_list_str + ")) " \
...
def update_dict_with_full_order(products):
    ...
    global DEBUG_PRODUCT_CATEGORIES
    

    # checks if the pair of products have any common L1 category
    def any_common_product_categories(p1, p2):
        common_categories = [c for c in p1['categories'] if c in p2['categories']]
        return len(common_categories) > 0
    ...
    for p in products:
        ...
        if DEBUG_MODE and p == DEBUG_PRODUCT:
            DEBUG_PRODUCT_CATEGORIES = products[p]['categories']
        ...
        for q in products:
            ...
            if q not in product_bought_with_dict[p]:
                ...
                if DEBUG_MODE:
                    ...
                    product_bought_with_dict[p][q] = {'score': 0, 'total_count': 0, 'category_matching': False, 'brand_matching': False, 'categories': products[q]['categories'], 'brand': products[q]['brand']}

            if DEBUG_MODE:
                product_bought_with_dict[p][q]['total_count'] += 1

            # stop here if the two products don't have at least 1 common L1 category
            if not any_common_product_categories(products[p], products[q]): continue

            if DEBUG_MODE:
                product_bought_with_dict[p][q]['category_matching'] = True
                product_bought_with_dict[p][q]['brand_matching'] = products[p]['brand'] == products[q]['brand']

            # Update the score. Same brand products get a boosted count
            product_bought_with_dict[p][q]['score'] += CROSS_BRAND_BOOSTING_FACTOR if products[p]['brand'] != products[q]['brand'] else BRAND_BOOSTING_FACTOR
...
def batch_pool_orders():
    ...
    for row in cursor.fetchmany(BATCH_SIZE):
        ...
        if current_order_id is None:
            ...
            current_order_products = { product_id: { 'categories': [category_id] if category_id else [], 'brand': brand } }
            continue

        # If order_id not changed, continue adding products to the 'current' dict
        if order_id == current_order_id:
            if product_id in current_order_products:
                # The orders SQL query will return a separate row for each L1 category that product belongs to, hence the 'appending' as they are received
                current_order_products[product_id]['categories'].append(category_id)
            else:
                current_order_products[product_id] = { 'categories': [category_id] if category_id else [], 'brand': brand }
            continue
        ...
        if order_id != current_order_id:
            ...
            # TODO not full proof, relying if null 
            #for product_id in current_order_products:
            #    if not current_order_products[product_id]['categories']: 
            #        parent_id = current_order_products[product_id]['parent_item_id']
            #        if item_product[parent_id]:
            #            current_order_products[product_id]['categories'] = current_order_products[item_product[parent_id]]['categories']
            ...
            current_order_id = order_id
            current_order_products = { product_id: { 'categories': [category_id], 'brand': brand } }
            continue
...
while (batch_pool_orders()):
    ...
    if DEBUG_MODE and DEBUG_PRODUCT_DATA_UPDATED:
        for product_id, bought_with in product_bought_with_dict.items():
            ...
            print('Top 5 (by total count) frequently bought details for product "' + str(product_id), '(' + DEBUG_PRODUCT_BRAND + ')', DEBUG_PRODUCT_CATEGORIES, '" so far')
            print('')
            print('Count, Category Match, Brand Match, Score, Product Details')
            for f_id, freq_info in sorted_bought_with_tuples:
                print(str(freq_info['total_count']) + ', ' + str(freq_info['category_matching']) + ', ' + str(freq_info['brand_matching']) + ', ' +  str(freq_info['score']),
                      'Product:', f_id, '(' + freq_info['brand'] + ')', freq_info['categories'])
            print('')
        DEBUG_PRODUCT_DATA_UPDATED = False
