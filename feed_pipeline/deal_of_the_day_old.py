...
cat3_ids = ['233', '234', '228', '231', '4140', '229', '235', '236', '232', '237', '6761', '239', '687', '240', '241',
            '242', '245', '1995', '247', '931', '244', '2440', '1513', '1514', '4036', '249', '263', '5079', '250',
            '251', '252', '253', '254', '255', '256', '257', '259', '260', '3277', '267', '269', '268', '270', '266',
            '271', '272', '4037', '283', '282', '222', '224', '284', '285', '933', '227', '5486', '2063', '521', '1649',
            '376', '691', '380', '379', '372', '1650', '1644', '2084', '2069', '288', '536', '2080', '289', '287',
            '226', '281', '225', '2065', '2066', '2067', '2068', '632', '286', '316', '317', '319', '2040', '320',
            '2041', '364', '363', '2444', '2043', '332', '331', '329', '346', '1214', '1222', '2046', '2746', '2045',
            '7305', '2089', '525', '583', '545', '550', '367', '551', '370', '1495', '554', '555', '572', '3859',
            '2092', '364', '1543', '1399', '1400', '1401', '1404', '1405', '41', '1517', '43', '44', '368', '369',
            '367', '370', '316', '319', '691', '374', '376', '283', '224', '2084', '1650', '1644', '975', '2057',
            '1673', '1676', '1675', '1546', '4874', '1664', '636', '637', '635', '638', '1274', '6919', '6917', '6922',
            '223', '1411', '1650', '962', '1392', '1388', '1387', '377', '371', '527', '547', '1393', '979', '1323',
            '7336', '971', '539', '571', '531', '1301', '529', '7313', '541', '1302', '7306', '524', '526', '528',
            '532', '533', '530', '544', '549', '552', '2108', '2109', '2111', '1559', '2096', '2107', '581', '535',
            '2095', '574', '573', '5746']
...
def get_intent_set():
  ...
  report['order_conversion_flag'] = (((report['o'] / report['ca']) >= ((report['o'] / report['ca']).quantile(0.2))) &
                                     ((report['o'] / report['ca']) <= (
                                       (report['o'] / report['ca']).quantile(quantile_upper_limit))))
  
  report['cart_conversion_flag'] = (
        ((report['ca'] / report['pv']) >= ((report['ca'] / report['pv']).quantile(quantile_lower_limit))) &
...
def get_rating_set():
  ...
  try:
    query = """select product_sku, product_id, rating_star, date(created_at), status_code from reviews_qna where status_code in ('Approved','Pending')  and created_at >='{0}'""".format(
      back_date_7_days)
...
def get_sales_data():
  ...
  product_sales_data['flag'] = (
        product_sales_data['count'] >= product_sales_data['count'].quantile(quantile_lower_limit))
...
def get_product_sku_data():
  ...
  for index, row in product_sku_raw_data.iterrows():
    if (row['sku_type'].lower()) in ['simple']:
      ...
      sku_details_map[str(row['sku'])] = {"sku": row["sku"], "type": (row['sku_type']).lower(),
...
def helper_function(weekly_category_wise_count_map, weekly_category_brand_count_map, weekly_brand_count_map, docs,
  ...
  for doc in new_docs:
    ...
    for l in l_list:
      for brand_ids in brand_id_list:
        ...
        if ((daily_category_count_map.get(l, 0) >= DAILY_CATEGORY_LIMIT) and (
            daily_category_count_map.get(l, 0) >= daily_categorywise_limits.get(l, 0))) or (
            (weekly_category_wise_count_map.get(l, 0) >= WEEKLY_CATEGORY_LIMIT) and (
            weekly_category_wise_count_map.get(l, 0) >= weekly_categorywise_limits.get(l, 0))):
          is_eligible = False
        if brand_ids and ((daily_brand_count_map.get(brand_ids, 0) >= DAILY_BRAND_LIMIT and (
            daily_brand_count_map.get(brand_ids, 0) >= daily_brandwise_limits.get(brand_ids, 0))) or (
                              weekly_brand_count_map.get(brand_ids,
                                                         0) >= WEEKLY_BRAND_LIMIT and weekly_brand_count_map.get(
                              brand_ids, 0) >= weekly_brandwise_limits.get(brand_ids, 0))):
          is_eligible = False
...
def get_dotd_products(dt):
  redshift_conn = PasUtils.redshiftConnection()
  query = "select distinct product_id,sku from deal_of_the_day_data where date_time >='{0}' and date_time <'{1}'".format(
    back_date_7_days, dt)
...
def get_final_eligible_product_list(docs, dt):
  ...
  final_list = helper_function(weekly_category_wise_count_map, weekly_category_brand_count_map, weekly_brand_count_map,
                               docs, already_dotd_products)
  return final_list
  ...
  for counter, doc in enumerate(new_docs[:30]):
    if doc.get('product_id'):
      ...
      query = "insert into deal_of_the_day_data values ('{0}', '{1}', '{2}', '{3}', '{4}', '{5}', '{6}')".format(
        product_id, sku, position, dt, sets, price, discount)
  ...
  for p in product_details:
    ...
    if pas_object.get(sku) and sku_details_map.get(sku):
      ...
      for key, value in sku_details.items():
        row[key] = value
      if row.get("is_in_stock", 0) and (not row.get('disabled', False)) and row.get('discount',
                                                                                    0) >= min_discount and row.get(
          'discount', 0) <= max_discount and row.get('product_id') and float(row.get('quantity', 0)) > 10:
